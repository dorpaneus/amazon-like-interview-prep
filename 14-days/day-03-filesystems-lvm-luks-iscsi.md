# Day 3 — Filesystems, LVM, LUKS, iSCSI (Thu Apr 30)

Today goes from the inode (Day 1) up the storage stack: physical/virtual disks → LVM → filesystem → mount. Each layer has a failure mode and a recovery procedure. Interviewers love this stack because it's where "things just broke" happens most.
...
| | ext4 | XFS |
|---|---|---|
| Default on | Older RHEL, Debian/Ubuntu | RHEL 7+, default for `/` |
...
**The XFS-can't-shrink trap**: real interview question. If you grow an XFS volume and need to shrink it, the answer is "you don't — back up, recreate smaller, restore." Cost of XFS's design tradeoff.
...
**Snapshots are copy-on-write** — when you snapshot an LV, the snapshot is initially empty pointers to the origin's blocks. Writes to the origin trigger a copy of the *old* block to the snapshot. Snapshot size needs to be ≥ expected change rate × time you'll keep it. **Snapshot full = snapshot becomes invalid**, not paused. This is a high-frequency interview gotcha.

**The metadata recovery point** — `vgcfgbackup` automatically saves VG metadata on every change to `/etc/lvm/archive/`. If you wipe a PV header by accident, `vgcfgrestore` brings it back. **This is the answer** to "you accidentally ran `dd` over the start of `/dev/sdb`, can you recover the LVM?" Yes — `vgcfgrestore -f /etc/lvm/archive/vg_data_*.vg vg_data` rewrites the metadata using the archive.
...
This procedure — **pvcreate with the original UUID + vgcfgrestore** — is the answer to "PV header damaged, recover the LVM."
...
sudo cryptsetup luksHeaderBackup /dev/vg_lab/lv_secret \
   --header-backup-file ~/luks-secret-header.bin
...
# Restore
sudo cryptsetup luksHeaderRestore /dev/vg_lab/lv_secret \
   --header-backup-file ~/luks-secret-header.bin
...
7. Two common causes: (1) deleted file held open by a process (`lsof | grep deleted`), (2) inode exhaustion — check `df -i`. Also possible: reserved blocks (ext4 reserves 5% for root by default).
